// Headless free-for-all: four factions with their perks, no steering, many seeds.
// Usage: node scripts/team-balance.mjs [seeds] [seconds]
...
const seeds = Number(process.argv[2] || 40), seconds = Number(process.argv[3] || 90);
...
for (let seed = 1; seed <= seeds; seed++) {
    ...
    for (let t = 0; t < seconds * 60; t++) sim.step();
    let best = null;
